...
using ARW.CodeGenerator;
using ARW.CodeGenerator.Model;
using ARW.CodeGenerator.Service;
using ARW.Common;
using ARW.Model;
...
namespace ARW.Admin.WebApi.Controllers
{
    ...
    public class CodeGeneratorController : BaseController
    {
        ...
        public IActionResult CodeGenerate([FromBody] GenerateDto dto)
        {
            ...
            //生成压缩包
            string zipReturnFileName = $"ARWAdmin.NET-{genTableInfo.TableComment}-{DateTime.Now:MMddHHmmss}.zip";
        ...
        [ActionPermissionFilter(Permission = "tool:gen:codeApi")]
        public IActionResult CodeGenerateApi([FromBody] GenerateDto dto)
        {
            if (dto?.TableId <= 0)
            {
                throw new CustomException(ResultCode.CUSTOM_ERROR, "请求参数为空");
            ...
            //生成代码到指定文件夹
            CodeGeneratorTool.GenerateApi(dto);
            //下载文件
            FileHelper.ZipGenCode(dto.ZipPath, dto.GenCodePath, zipReturnFileName);
